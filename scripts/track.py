        ...
        with open(result_path, 'w') as f_out:
            for meta, img, img0 in data_loader:
                ...
                for cls_id in range(self.num_cls):
                    if cls_id in self.skip_set:
                        continue
                    # class_remap: passthrough (returns cls_id) khi không có merge
                    # profile active ('10class'/'5class'/'4class'); trả về target
                    # id (đã merge) hoặc None (drop) khi eval_mode là *_merge.
                    target_cls_id = class_remap.remap_raw_cls_id(cls_id)
                    ...
                    for tlwh, tid, sc in zip(
                        ...
                        f_out.write(_RESULT_FMT.format(
                            frame  = frame_id,
                            # Offset LUÔN theo RAW cls_id (không theo target sau merge)
                            # để giữ tính duy nhất giữa các raw class merge vào cùng
                            # 1 target (vd van#3 và truck#3 không bị coi là cùng object).
                            # Phải khớp công thức offset GT trong io.py.
                            id     = tid + cls_id * _CLS_ID_OFFSET,
                            x1=tlwh[0], y1=tlwh[1], w=tlwh[2], h=tlwh[3],
                            score  = sc,
                            cls_id = target_cls_id + 1,   # ghi class ĐÃ MERGE (1-indexed)
                        ))
