...
class SequenceRunner:
    ...
    def __init__(self, opt, frame_rate: int = 30):
        ...
        # ── MODEL class (0-indexed, taxonomy 7-class) -> (target_cls_id_0idx,
        #    target_name) cho các class ĐƯỢC GIỮ. Class bị drop (vd bicycle,
        #    motor ở 5class_merge_benchmark) KHÔNG có mặt trong map nên sẽ
        #    không ghi ra file kết quả và không được vẽ trong visualize.
        #    set_eval_mode()/set_merge_profile() đã chạy ở __main__ trước khi
        #    SequenceRunner được khởi tạo, nên profile đã active tại đây.
        self.kept_pred_map = {}
        for raw_pred in range(self.num_cls):
            tgt = class_remap.remap_pred_cls_id(raw_pred)
            if tgt is not None:
                self.kept_pred_map[raw_pred] = (tgt, class_remap.target_id2name(tgt))
    ...
    def _visualize(self, img0, tlwhs, tids, scores, frame_id, save_dir, show_image):
        # Chỉ vẽ các class ĐƯỢC GIỮ (kept_pred_map); class bị drop
        # (bicycle/motor) bị loại hoàn toàn khỏi hình. Ảnh nền là ảnh đã
        # tiền xử lý sẵn từ dataset (chỉ vùng ignore thực sự mới bị bôi đen),
        # track.py KHÔNG bôi đen thêm gì.
        vis_tlwhs   = defaultdict(list)
        vis_tids    = defaultdict(list)
        vis_scores  = defaultdict(list)
        vis_names   = {}
        for cls_id, (tgt, name) in self.kept_pred_map.items():
            if tlwhs.get(cls_id):
                vis_tlwhs[cls_id]  = tlwhs[cls_id]
                vis_tids[cls_id]   = tids[cls_id]
                vis_scores[cls_id] = scores[cls_id]
                vis_names[cls_id]  = name

        online_im = vis.plot_tracks(
            image        = img0,
            tlwhs_dict   = vis_tlwhs,
            obj_ids_dict = vis_tids,
            num_classes  = self.num_cls,
            scores       = vis_scores,
            frame_id     = frame_id,
            fps          = 1.0 / max(1e-5, self.timer.average_time),
            cls_id2name  = vis_names,
        )
        if show_image:
            cv2.imshow('online_im', online_im)
        if save_dir:
        ...
        with open(result_path, 'w') as f_out:
            for meta, img, img0 in data_loader:
                ...
                for cls_id in range(self.num_cls):
                    if cls_id in self.skip_set:
                        continue
                    # PREDICTION side: model output là taxonomy 7-class.
                    # kept_pred_map giữ các class map được sang 5-class target;
                    # class bị drop (bicycle/motor) KHÔNG có trong map -> bỏ qua
                    # (không ghi file, không thành FP vì GT đã loại 2 class này).
                    if cls_id not in self.kept_pred_map:
                        continue
                    target_cls_id, _ = self.kept_pred_map[cls_id]
                    for tlwh, tid, sc in zip(
                            tlwhs[cls_id], tids[cls_id], tscores[cls_id]):
                        if tid < 0:
                            continue
                        f_out.write(_RESULT_FMT.format(
                            frame  = frame_id,
                            # Offset theo RAW model cls_id (7-class) để giữ tính
                            # duy nhất giữa các class trong accumulator prediction.
                            id     = tid + cls_id * _CLS_ID_OFFSET,
                            x1=tlwh[0], y1=tlwh[1], w=tlwh[2], h=tlwh[3],
                            score  = sc,
                            cls_id = target_cls_id + 1,   # ghi class 5-class (1-indexed)
                        ))
...
if __name__ == '__main__':
    ...
    _mode_info = {
        '10class': 'all 10 VisDrone classes (official)',
        '5class':  'pedestrian / car / van / truck / bus (AMOT protocol, SKIP)',
        '4class':  'person / car / motorcycle / bicycle (competition, SKIP)',
        '5class_merge_benchmark':   'pedestrian / car / truck / tricycle / bus (MERGE, drop bicycle+motor)',
        '5class_merge_competition': 'person / car / truck / motorcycle / bicycle (MERGE, drop tricycle+awning-tricycle+bus)',
    }
    print(f'[Eval] {eval_mode}: {_mode_info[eval_mode]}')
