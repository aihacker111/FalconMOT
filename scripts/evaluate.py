"""
evaluate.py — VisDrone-correct mAP detection evaluation on VisDrone test-dev.

Implements the following protocol (nhất quán với gen_dataset_visdrone.py):
  - maxDets = 500  (VisDrone images có thể có 200-600+ objects mỗi frame)
  - ignore regions: score=0 / cls_id=0 hoặc 11 → detection trùng vùng này
    không bị tính là FP  (intersection/det_area >= 0.5)
  - Không filter truncation: giữ lại tất cả GT kể cả object bị cắt nhiều
    để mAP nhất quán với training data (gen_dataset cũng giữ tất cả truncation)
...
def parse_visdrone_gt(ann_path):
    ...
    Filtering nhất quán với gen_dataset_visdrone.py:
        - score=0 OR cls_id=0 OR cls_id=11 → ignore region
        - Không filter occlusion  : tất cả object bị che đều là GT hợp lệ
        - Không filter truncation : tất cả object bị cắt đều là GT hợp lệ
          (gen_dataset cũng giữ chúng trong training → mAP nhất quán)
    """
    gt     = defaultdict(list)
...
def main(opt):
    ...
    for seq in seq_bar:
        ...
        for frame_id, (path, img, img0) in enumerate(loader, start=1):
            ...
            # ── Decode predictions — giống EdgeCrafter PostProcessor ───────
