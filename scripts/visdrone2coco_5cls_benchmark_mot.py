...
Chuyển đổi VisDrone2019-MOT sang COCO JSON, merge về 5 class chuẩn (benchmark):
    1: pedestrian, 2: car, 3: truck, 4: tricycle, 5: bus
    (Drop hoàn toàn: bicycle, motor)

Khớp eval_mode='5class_merge_benchmark' trong opts.py / track.py / class_remap.py.

Logic ảnh: KHÔNG bôi đen (blackout) các class bị drop (bicycle/motor) — giữ
nguyên pixel; chỉ bôi đen các vùng IGNORE thực sự của VisDrone (score==0 hoặc
cls==0 (unlabeled) hoặc cls==11 (others)).

CHANGELOG (so với bản trước):
    * Mỗi image record GIỜ ghi thêm 'seq_id' và 'frame_id'. Đây là field BẮT
      BUỘC để LoadCocoSequencesForTracking (track.py) gom frame theo sequence
      và sort theo frame_id THẬT. Bản trước thiếu 2 field này nên loader gán
      frame_id=0 cho mọi frame -> thứ tự loạn, kết quả tracking sai.
    * Thêm guard bbox w>0 & h>0 trước khi tạo annotation.
    * Parse annotation an toàn (cắt phần dư không đủ 10 cột).
    * --splits cấu hình được (mặc định chỉ 'test-dev' như benchmark cũ).
...
# ── Class mapping 5 class (1-indexed target) ─────────────────────────────────
TARGET_CLS_MAP = {
    1: 'pedestrian', 2: 'car', 3: 'truck', 4: 'tricycle', 5: 'bus'
}
NUM_CLASSES = len(TARGET_CLS_MAP)

# VisDrone gốc 1-indexed -> 5 class target (None = DROP)
#   1 pedestrian  2 people  3 bicycle  4 car   5 van
#   6 truck       7 tricycle 8 awning-tricycle 9 bus 10 motor
CLASS_MAPPING = {
    1: 1, 2: 1,      # pedestrian, people     -> pedestrian (1)
    3: None,         # bicycle                -> DROP
    4: 2,            # car                    -> car        (2)
    5: 3, 6: 3,      # van, truck             -> truck      (3)
    7: 4, 8: 4,      # tricycle, awning-tri   -> tricycle   (4)
    9: 5,            # bus                    -> bus        (5)
    10: None,        # motor                  -> DROP
}
...
def convert_split(src_root, dst_root, split, workers=8, overwrite=False):
    ...
    for seq in tqdm(sorted(os.listdir(seq_dir)), desc=f'[{split}]'):
        ...
        # Object hợp lệ: giữ class merge được, drop bicycle/motor (KHÔNG bôi đen).
        ...
        # Rank track-id theo tuple (raw_cls, raw_tid): van#3 != truck#3 dù cùng
        # merge vào 'truck' -> hai object khác nhau không bị gán chung track_id.
    ...
    print(f'  [Info] Dropped {total_dropped:,} bicycle/motor objects (pixels kept)')
    print(f'  -> {out_json}')


if __name__ == '__main__':
    ap = argparse.ArgumentParser(
        description='VisDrone2019-MOT -> COCO JSON, 5-class benchmark merge '
    ...
    for split in args.splits:
        ...
        src = os.path.join(args.visdrone_root, f'visdrone2019-{split}')
