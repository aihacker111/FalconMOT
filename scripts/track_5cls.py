...
class ECDetSequenceRunner5cls:
    ...
    def __init__(self, opt, frame_rate: int = 30):
        ...
        self.model = opt.model
        # Quy ước resize: plain-resize (mặc định repo) hay letterbox — quyết định
        # cách map dense-map<->orig ở set_dense. Mặc định False = plain-resize.
        self._letterbox = getattr(opt, 'letterbox', False)
        # UAM cần dense appearance map từ model. Mặc định BẬT để khớp tracker mới.
        if getattr(opt, 'use_appearance_motion', True):
            _m = getattr(self.model, 'module', self.model)
            _m.return_reid_dense = True
        ...
        with open(result_path, 'w') as f_out:
            for frame_id, img, img0 in seq_loader:
                ...
                if 'reid_dense' in output:
                    net_w, net_h = self._net_wh
                    if self._letterbox:
                        # letterbox → tỉ lệ đẳng hướng + pad ở giữa
                        r = min(net_h / orig_h, net_w / orig_w)
                        rx = ry = r
                        pad_w = (net_w - orig_w * r) * 0.5
                        pad_h = (net_h - orig_h * r) * 0.5
                    else:
                        # plain-resize (repo này) → tỉ lệ theo từng trục, không pad
                        rx, ry = net_w / orig_w, net_h / orig_h
                        pad_w = pad_h = 0.0
                    self.tracker.set_dense(
                        output['reid_dense'],            # [C,H,W]
                ...
                online_targets = self.tracker.update(dets_5cls)
