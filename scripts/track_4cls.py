"""track_4cls.py — Tracking + MOTMetrics eval trên 4-class competition.
...
Pipeline:
    LoadCocoSequencesForTracking (COCO JSON)
        ...
        -> CocoGTEvaluator.eval_file()     ← GT từ COCO JSON
...
class SequenceRunner4cls:
    """Runner với 7->4 class remap built-in.

      1. Tracker init với num_classes=NUM_CLS_EVAL_4 (track-id offset theo 4 class).
      2. Sau decode (7cls), remap_dets_7cls_to_4cls() drop truck/tricycle/bus và
         shift class index trước khi đưa vào tracker.
      3. Khi ghi result file, cls_id là 4-cls index (1-indexed).
      4. Query Appearance-Motion (UAM) giữ nguyên — dense map wiring không đổi.
    """
    ...
    def run(self, seq_loader, result_path: str,
        ...
        """Chạy một sequence, ghi MOT result file theo 4-cls scheme."""
        if save_dir:
