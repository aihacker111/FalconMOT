"""track_ECDet.py — multi-class tracking inference + evaluation for FalconJDE.

Pipeline:
    LoadImagesForTracking -> ECDetSequenceRunner.run() -> incremental MOT file
                          -> Evaluator.eval_file() -> motmetrics MOTA / IDF1

Global track id = per-class track_id + cls_id_0indexed * 1_000_000
    VisDrone track ids are per-class (pedestrian #1 != car #1); the offset
    keeps ids unique inside a single motmetrics accumulator. The GT side
    (io.py) applies the same offset.
"""
...
class ECDetSequenceRunner:
    """Model + tracker wrapper for a single sequence.

    - Incremental write: results are flushed per frame (no RAM accumulation).
    - Cached orig_sizes: the size tensor is created once per sequence.
    - Vectorised box filter and single-pass track collection.
    """
    ...
    def __init__(self, opt, frame_rate: int = 30):
        ...
        self.tracker = MCJDETracker(opt, frame_rate)
        self.timer = Timer()
        self._orig_sizes = None   # cached once per sequence

    def _decode_detections(self, res: dict) -> defaultdict:
        """Postprocessor output -> per-class list of MCTrack detections."""
        ...
        for i in valid:
            ...
            dets[cls_id].append(MCTrack(tlwh, float(scores_np[i]), emb, self.num_cls, cls_id))
        return dets
    ...
    def run(self, data_loader, result_path: str, save_dir=None, show_image: bool = False):
        """Run one sequence end-to-end, writing the MOT result file incrementally.
        ...
        with open(result_path, 'w') as f_out:
            for _, img, img0 in data_loader:
                ...
                # Incremental write (no in-memory results_dict).
                for cls_id in range(self.num_cls):
                    if cls_id in self.skip_set:
                        continue
                    for tlwh, tid, sc in zip(tlwhs[cls_id], tids[cls_id], tscores[cls_id]):
                        if tid < 0:
                            continue
                ...
                if show_image or save_dir:
                    self._visualize(img0, tlwhs, tids, tscores, frame_id, save_dir, show_image)
...
def main(opt, data_root: str, ann_root: str, seqs,
    ...
    runner = ECDetSequenceRunner(opt)
