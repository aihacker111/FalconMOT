...
def run_track_eval(model, opt, val_ann_file: str, val_img_root: str) -> dict:
    ...
    Differences from the standalone script (by design):
      ...
      * GMC (sparse optical flow, CPU) is OFF by default: it costs 5–15 ms of
        serial CPU work per frame and would stall the GPU. Re-enable with
        ``--track_val_gmc 1`` if the val split has strong camera motion.
      * motmetrics accumulation runs on a background thread per sequence, so
        metric bookkeeping overlaps GPU inference of the next sequence.
    ...
    use_fp16   = bool(int(getattr(opt, 'track_val_fp16', 1))) and opt.device.type == 'cuda'
    use_gmc    = bool(int(getattr(opt, 'track_val_gmc', 0)))
    n_threads  = int(getattr(opt, 'track_val_loader_threads', 4))
    prefetch   = int(getattr(opt, 'track_val_prefetch', 8))

    def _accumulate_seq(seq_id, frame_buf):
        """Run motmetrics accumulation for one finished sequence (worker thread)."""
        ev = CocoGTEvaluator.from_gt_dict(seq_id, gt_by_seq.get(seq_id, {}))
        for fid, tlwhs, tids in frame_buf:
            ev.eval_frame(fid, tlwhs, tids)
        return ev.acc

    eval_pool    = ThreadPoolExecutor(max_workers=2)
    eval_futures = []          # [(seq_id, Future)] — keeps sequence order
    t0, n_frames = time.time(), 0
    total_frames = sum(src.num_frames(s) for s in src.seqs)
    pbar = tqdm(total=total_frames, desc='[track-eval]', unit='f', dynamic_ncols=True)
    ...
    try:
        for seq_id in src.seqs:
            tracker.reset()
            if use_gmc:
                # reset() does not clear GMC state — flush it so frame 1 of this
                # sequence is not warped against the last frame of the previous one.
                tracker._curr_img = None
                tracker.gmc.prevFrame = None
                tracker.gmc.initializedFirstFrame = False
            frame_buf = []     # (frame_id, tlwhs, tids) — evaluated off-thread
            sizes_cache = {}   # (h, w) -> tensor; constant within a sequence
            ...
            for frame_id, img, img0 in reader:
                orig_h, orig_w = img0.shape[:2]
                sizes = sizes_cache.get((orig_h, orig_w))
                if sizes is None:
                    sizes = torch.tensor([[orig_h, orig_w]], device=opt.device)
                    sizes_cache[(orig_h, orig_w)] = sizes
                blob  = torch.from_numpy(img[None]).to(opt.device, non_blocking=True)
                ...
                # GMC is pure serial CPU cost (sparse optical flow) — only feed
                # the raw frame when explicitly enabled for validation.
                if use_gmc:
                    tracker.set_image(img0)
                ...
                frame_buf.append((int(frame_id), trk_tlwhs, trk_ids))
            ...
            # Accumulate this sequence's metrics off the GPU loop.
            eval_futures.append((seq_id, eval_pool.submit(_accumulate_seq, seq_id, frame_buf)))
    finally:
        pbar.close()
        core_model.return_reid_dense = False
        model.train()
    ...
    try:
        accs      = [fut.result() for _, fut in eval_futures]
        seq_names = [sid for sid, _ in eval_futures]
    finally:
