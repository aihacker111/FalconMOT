...
Convert VisDrone2019-MOT to COCO JSON, merged into 4 classes (competition):
    1: person, 2: car, 3: motorcycle, 4: bicycle
        (Fully dropped: van, truck, tricycle, awning-tricycle, bus)

Matches eval_mot_4cls.py (7cls model -> 4cls eval remap) and the corresponding class names.

Image logic: do NOT black out the dropped classes (van/truck/tricycle/bus) --
their pixels are kept; only the genuine VisDrone IGNORE regions are blacked out (score==0 or
cls==0 (unlabeled) or cls==11 (others)).

Each image record stores 'seq_id' and 'frame_id' (REQUIRED so
LoadCocoSequencesForTracking groups frames by sequence and sorts by real frame index).
...
def _parse_ann_file(path: str) -> np.ndarray:
    """Parse a VisDrone annotation -> (N, 10) int32. Trim any trailing columns beyond 10."""
...
def _process_frame(src_path: str, dst_path: str, ignore_boxes: list,
    ...
    """Read the source frame, black out ONLY the genuine ignore regions, write to dst. Return (H, W)."""
...
def convert_split(src_root, dst_root, split, workers=8, overwrite=False):
    ...
    img_id, ann_id = 0, 0
    track_start = [0] * NUM_CLASSES          # global per-class track-id offset
    total_dropped = 0
    ...
    for seq in tqdm(sorted(os.listdir(seq_dir)), desc=f'[{split}]'):
        ...
        # Genuine ignore region -> always blacked out.
        ignore_by_frame = defaultdict(list)
        for row in arr[is_ignore]:
            ignore_by_frame[int(row[0])].append(row[2:6])

        # Valid object: keep the mergeable classes, drop van/truck/tricycle/bus
        # (NOT blacked out -- pixels kept).
        ...
        # even if both merge into 'person' -> two distinct objects are not assigned the same id.
        ...
        # Build COCO records (sequential, deterministic order).
