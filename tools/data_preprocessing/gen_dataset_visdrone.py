...
Filtering rules (consistent with evaluate.py):
    Ignore regions (blacked out on the image, not GT):
            - score == 0          : annotator marked it to be ignored
            - cls_id == 0         : region was left unlabeled
            - cls_id == 11        : "others" category outside the 10 classes
    Valid GT objects (written to the label file):
        - score == 1 AND 1 <= cls_id <= 10
            - No occlusion filtering   : eval also counts every occluded object
            - No truncation filtering  : kept for full mAP (consistent with eval_det_visdrone.py)
"""
...
def draw_ignore_regions(img, boxes):
    """Black out the ignore regions on the image (in-place).
...
def gen_dot_train_file(data_root, rel_path, out_root, f_name='detrac.train'):
    """Create an index file listing the relative image paths."""
...
def gen_track_dataset(src_root, dst_root, viz_root=None):
    """Convert a VisDrone MOT split to JDE format (images + labels_with_ids).

    Args:
            src_root: VisDrone MOT split root
                                (must contain sequences/ and annotations/)
            dst_root: output directory (created if missing)
            viz_root: if set, save visualization images with drawn bboxes
    """
    ...
    # global track-id offset per class (accumulated across sequences)
    ...
    for seq in tqdm(seq_names, desc='sequences'):
        ...
        # All three cases are blacked out on the image and excluded from GT.
        is_ignore = (
        ...
        # Do not filter occlusion or truncation so that:
        #   1. the model learns to detect occluded/truncated objects too
        #   2. eval_det_visdrone.py also does no filtering -> consistent mAP
        is_valid = (
            (seq_label_array[:, 6] == 1) &
            (seq_label_array[:, 7] > 0)  &
        ...
        for fr_id, fr_labels in seq_objs_label_dict.items():
            ...
            # Black out ignore regions (cls_id=0, cls_id=11, score=0)
        ...
        # Update the global track-id offset after each sequence
