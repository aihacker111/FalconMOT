...
Convert VisDrone2019-MOT to COCO JSON, merged into the 5-class benchmark:
    1: pedestrian, 2: car, 3: van, 4: truck, 5: bus
        (Here: pedestrian = pedestrian + people. Other classes dropped: bicycle, tricycle, awning-tricycle, motor)

Image logic: do NOT black out the dropped classes -- their pixels are kept;
only the genuine VisDrone IGNORE regions are blacked out (score==0 or cls==0 or cls==11).

Track-ID guarantee:
        - Uses TrackIDManager to assign global IDs safely and without collisions
      khi merge class (vd: people -> pedestrian).
        - Each image record carries `seq_id` and `frame_id` for LoadCocoSequencesForTracking.
"""
