...
Usage:
    python src/gen_dataset_visdrone_coco.py \
        --visdrone_root /workspace \
        --output_root  /workspace/VisDrone2019-COCO \
        --splits train val \
        --workers 8
"""
...
def convert_split(src_root: str, dst_root: str, split: str,
    ...
    img_id      = 0
    ann_id      = 0
    track_start = [0] * 10   # global per-class ID offset across sequences
    ...
    for seq in tqdm(seq_names, desc=f'[{split}]'):
        ...
        # Group ignore boxes and valid rows by frame_id (Python int keys)
        ...
        # ── 2. Build track-id rank dict — O(1) lookup per annotation ─────
        track_ids_per_cls: dict[int, set] = defaultdict(set)
        for rows in valid_by_frame.values():
            for row in rows:
                track_ids_per_cls[int(row[7]) - 1].add(int(row[1]))

        # rank_map[cls_id_0][raw_tid] → local_rank (0-indexed within seq)
        rank_map = {
            cls_id: {tid: rank for rank, tid in enumerate(sorted(ids))}
            for cls_id, ids in track_ids_per_cls.items()
        }
        seq_n_ids = {cls_id: len(ids) for cls_id, ids in track_ids_per_cls.items()}

        tqdm.write(f'  {seq}: ' + '  '.join(
            f'{CLS_MAP[c+1]}={n}' for c, n in sorted(seq_n_ids.items())))

        # ── 3. Parallel image I/O ─────────────────────────────────────────
        frame_ids   = sorted(valid_by_frame.keys())
        frame_sizes: dict[int, tuple] = {}   # fr_id → (H, W)
        ...
        for fr_id in frame_ids:
            ...
            images_list.append({
                ...
                'width':    W,
                'seq_id':   seq,
                'frame_id': fr_id,           # already Python int
            })

            for row in valid_by_frame[fr_id]:
                raw_cls_1 = int(row[7])
                cls_id_0  = raw_cls_1 - 1
                raw_tid   = int(row[1])
                ...
                local_rank = rank_map[cls_id_0][raw_tid]
                track_id   = local_rank + track_start[cls_id_0]

                anns_list.append({
                    'id':          ann_id,
                    'image_id':    img_id,
                    'category_id': raw_cls_1,
                    'bbox':        [x1, y1, bw, bh],
                    'area':        bw * bh,
                    'iscrowd':     0,
                    'track_id':    track_id,
    ...
    for c in sorted(max_tid):
        print(f'    {CLS_MAP[c+1]}: {max_tid[c]} IDs')
...
def main():
    ap = argparse.ArgumentParser(
        description='Convert VisDrone2019-MOT → COCO JSON for DEIM-JDE training.')
