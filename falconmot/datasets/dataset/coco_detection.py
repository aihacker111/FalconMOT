...
class VisDroneCocoDataset(torch.utils.data.Dataset):
    ...
    def _partner_id(self, img_id, gap):
        """Lấy id của frame cùng sequence cách nhau `gap` bước."""
        seq, pos = self._id_to_pos.get(img_id, (None, 0))
        if seq is None:
            return None
        ids = self._seq_to_ids[seq]
    ...
    def _getitem_pair(self, idx):
        """
        Lấy ra dict chứa cả frame t và t+1 với Geometric Augmentation ĐỒNG BỘ TUYỆT ĐỐI.
        Tránh lỗi sụp đổ tọa độ trong tracking xuyên frame do augmentation lệch pha.
        """
        img_id = self.img_ids[idx]
        info   = self.img_info[img_id]
        pid    = self._partner_id(img_id, self.qam_pair_gap)
        ...
        img_a, lab_a = self._load_raw(img_id)
        if pid is None:
            img_b, lab_b = img_a.copy(), lab_a.copy()
            pid_exists = False
        else:
            img_b, lab_b = self._load_raw(pid)
            pid_exists = True
        ...
        # 1. Photometric Augment (Độc lập để mạng học được tính bất biến ánh sáng giữa các frame)
        if do_aug:
            augment_hsv(img_a, fraction=0.50)
            augment_hsv(img_b, fraction=0.50)

        # 2. Geometric Augment (Bắt buộc phải ĐỒNG BỘ ma trận chung cho cả 2 frame)
        H, W = img_a.shape[:2]
        if do_aug and random.random() < 0.8:
            # Sinh ma trận Affine ĐỒNG BỘ (Scale + Translate) cho cả t và t+1
            scale = random.uniform(0.5, 1.2)
            tx = random.uniform(-0.1, 0.1) * self.width
            ty = random.uniform(-0.1, 0.1) * self.height
            
            M = np.zeros((2, 3), dtype=np.float32)
            M[0, 0] = scale * (self.width / W)
            M[1, 1] = scale * (self.height / H)
            M[0, 2] = (self.width / 2.0) * (1.0 - scale) + tx
            M[1, 2] = (self.height / 2.0) * (1.0 - scale) + ty
            
            img_a = cv2.warpAffine(img_a, M, (self.width, self.height), flags=cv2.INTER_LINEAR, borderValue=(0,0,0))
            img_b = cv2.warpAffine(img_b, M, (self.width, self.height), flags=cv2.INTER_LINEAR, borderValue=(0,0,0))
            
            def warp_labels(labs):
                if len(labs) == 0: return labs
                out = labs.copy()
                cx = labs[:, 2] * W
                cy = labs[:, 3] * H
                w  = labs[:, 4] * W
                ...
                # Biến đổi tọa độ theo cùng ma trận M rồi normalize lại
                out[:, 2] = (M[0, 0] * cx + M[0, 2]) / self.width
                out[:, 3] = (M[1, 1] * cy + M[1, 2]) / self.height
                out[:, 4] = (M[0, 0] * w) / self.width
                out[:, 5] = (M[1, 1] * h) / self.height
                
                # Lọc bỏ các object bị đẩy văng ra ngoài khung hình
                valid = (out[:, 2] > 0) & (out[:, 2] < 1) & (out[:, 3] > 0) & (out[:, 3] < 1)
                return out[valid]
                
            lab_a = warp_labels(lab_a)
            lab_b = warp_labels(lab_b)
        else:
            # Plain resize
            img_a = cv2.resize(img_a, (self.width, self.height), interpolation=cv2.INTER_AREA)
            img_b = cv2.resize(img_b, (self.width, self.height), interpolation=cv2.INTER_AREA)

        # 3. Flip ngang ĐỒNG BỘ
        if do_aug and random.random() > 0.5:
            img_a = np.fliplr(img_a)
            img_b = np.fliplr(img_b)
            if len(lab_a) > 0: lab_a[:, 2] = 1.0 - lab_a[:, 2]
            if len(lab_b) > 0: lab_b[:, 2] = 1.0 - lab_b[:, 2]

        # 4. Che khuất và Tạo nhiễu (ĐỘC LẬP - Ép ReID/QAM học xuyên qua vật cản)
        if do_aug and self.use_obj_occlusion:
            img_a, lab_a = object_aware_occlusion(img_a, lab_a, p=self.obj_occ_prob, occ_obj_frac=self.obj_occ_frac, mode=self.obj_occ_mode)
            img_b, lab_b = object_aware_occlusion(img_b, lab_b, p=self.obj_occ_prob, occ_obj_frac=self.obj_occ_frac, mode=self.obj_occ_mode)
        if do_aug and self.use_random_erasing:
            img_a, lab_a = random_erasing(img_a, lab_a, p=self.re_prob)
            img_b, lab_b = random_erasing(img_b, lab_b, p=self.re_prob)
        if do_aug and self.use_gridmask:
            img_a = self._gridmask(img_a, p=self.gridmask_prob)
            img_b = self._gridmask(img_b, p=self.gridmask_prob)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if not mosaic_used:
            ...
            # 4. Chuyển đổi nhãn về pixel theo tọa độ canvas
            if len(labels) > 0:
                lbs = labels.copy()
                lbs[:, 2] = (labels[:, 2] - labels[:, 4] * 0.5) * self.width
                lbs[:, 3] = (labels[:, 3] - labels[:, 5] * 0.5) * self.height
