...
"""Multi-class JDE tracker (UAM association).

`MCJDETracker` is model-decoupled: `update()` nhận detections đã decode
(`dict[cls_id] -> list[MCTrack]`) và chạy cascade ByteTrack-style theo từng lớp.

Association bước 1 dùng UAM (Uncertainty-Aware appearance-Motion):
  • APPEARANCE / ID  : sparse `emb` (do ReID head huấn luyện trực tiếp) -> cosine
                       cost (matching.embedding_distance). ĐÂY LÀ CUE ĐỊNH DANH.
  • MOTION           : dense appearance map -> correlation localisation kèm
                       covariance ĐO ĐƯỢC, fuse với Kalman bằng inverse-variance.
  • GATE             : Mahalanobis χ² (hằng số thống kê) HOẶC IoU; rồi trần cosine.

Hai cue KHÔNG trùng vai: `emb` trả lời "ai", dense map trả lời "ở đâu".
So với QAM cũ, UAM bỏ hẳn: am_kappa / am_beta / am_w_app / am_w_iou /
proximity_thresh / motion_gate (độ bất định nay ĐO chứ không GIẢ ĐỊNH).

Khi không có dense map ở frame hiện tại -> tự fallback IoU + appearance
(matching.fuse_score_three), không cần tham số phụ.
"""
...
from falconmot.tracker import matching
from falconmot.tracker import uam_ops as uam
from falconmot.tracker import appearance_motion as am
from falconmot.tracking_utils.kalman_filter import KalmanFilter
from falconmot.tracking_utils.gmc import GMC
from .basetrack import MCBaseTrack, TrackState

# VisDrone 10-class id -> name (chỉ để visualise / debug)
id2cls = {
    0: 'pedestrian', 1: 'people',   2: 'bicycle',  3: 'car',
    4: 'van',        5: 'truck',    6: 'tricycle', 7: 'awning-tricycle',
    8: 'bus',        9: 'motor',
...
class MCTrack(MCBaseTrack):
    ...
    def __init__(self, tlwh, score, temp_feat, num_classes, cls_id, buff_size=30):
        ...
        # --- APPEARANCE / ID: sparse emb (cue định danh, dùng cho cosine cost) ---
        self.smooth_feat = None
        self.alpha = 0.9
        self.update_features(temp_feat)
        self.features = deque([], maxlen=buff_size)

        # --- MOTION: template value-space lấy mẫu từ dense map (chỉ cho UAM) ---
        self.template = None
        self.pred_xy = None    # tâm UAM dự đoán (orig coords), nếu có

        self.curr_tlwh = np.asarray(tlwh, dtype=np.float32)
        self.tlwh_deque = deque([], maxlen=30)

    def update_template(self, feat, alpha=0.9):
        """EMA template value-space (L2-normalised) — dùng cho correlation motion."""
        if feat is None:
            return
        feat = np.asarray(feat, dtype=np.float32)
        n = np.linalg.norm(feat)
    ...
    def update_features(self, feat, alpha=None):
        """L2-normalise sparse emb + cập nhật EMA `smooth_feat` (cue ID)."""
        feat = np.asarray(feat, dtype=np.float32)
        n = np.linalg.norm(feat)
        if n > 0:
            feat = feat / n
...
class MCJDETracker(object):
    """Per-class ByteTrack-style tracker với UAM association."""
    ...
    def __init__(self, opt, frame_rate=30):
        ...
        self.use_gmc = getattr(opt, 'use_gmc', True)
        self.gmc = GMC(method='sparseOptFlow', verbose=[None, False]) if self.use_gmc else None
        self._curr_img = None

        # ── UAM config — chỉ còn các nút thật sự ──
        self.use_am   = getattr(opt, 'use_appearance_motion', True)
        self.uam_tau  = getattr(opt, 'am_tau', 0.1)          # độ phân giải response
        self.uam_chi2 = getattr(opt, 'uam_chi2', 9.21)       # χ²₂ @ 0.99 (hằng số)
        self.uam_cos  = getattr(opt, 'uam_cos_thresh', 0.4)  # trần cosine (NÚT CHÍNH)
        self.uam_iou_gate = getattr(opt, 'uam_iou_gate', 0.7)

        # state dense per-frame (set qua set_dense())
        self._dense_hat = None    # [C,H,W] map đã L2-normalise (torch)
        self._dense_raw = None    # [C,H,W] map thô (torch, để sample template)
        self._tf = None           # (stride, ratio_x, ratio_y, pad_w, pad_h)

    # ----- inputs per-frame -----
    def set_image(self, img):
        """Frame BGR gốc (cho GMC). Gọi trước update()."""
        self._curr_img = img

    def set_dense(self, reid_dense, stride, ratio_x, ratio_y, pad_w=0.0, pad_h=0.0):
        """Dense appearance map + phép biến đổi net<->orig.

        QUY ƯỚC TOẠ ĐỘ phải KHỚP với cách postprocessor decode box:
          • plain resize (repo này): ratio_x=net_w/orig_w, ratio_y=net_h/orig_h,
            pad_w=pad_h=0  (anisotropic, KHÔNG letterbox).
          • letterbox:               ratio_x=ratio_y=min(net/orig), pad ở giữa.
        (Lỗi cũ ở caller: chọn nhánh letterbox theo `_net_hw is not None` — luôn
         True. Hãy chọn theo CỜ letterbox tường minh, xem README ở cuối file.)
        """
        if reid_dense is None:
            self._dense_hat = self._dense_raw = self._tf = None
            return
    ...
    def _sample_det_templates(self, dets):
        """Lấy template MOTION của mỗi detection từ dense map tại tâm box.

        Lưu ý: đây CHỈ phục vụ correlation motion, KHÔNG thay sparse emb (vẫn là
        cue ID cho embedding_distance).
        """
        if self._dense_raw is None or len(dets) == 0:
            return
        import torch
    ...
    def _uam_predict(self, pool):
        """Vị trí fuse mỗi track: Kalman ⊕ correlation (inverse-variance).

        Returns:
            means : (T,2) tâm đã fuse (orig coords).
            covs  : (T,2,2) covariance đã fuse (orig px^2).
        """
        import torch
        T = len(pool)
        means = np.zeros((T, 2), dtype=np.float32)
        ...
        # 1) Kalman position mean + covariance (đo trong orig coords)
        for i, t in enumerate(pool):
            pm, pc = self.kalman_filter.project(t.mean, t.covariance)
            means[i] = pm[:2]
            covs[i] = pc[:2, :2]
            # Track coast càng lâu -> dự đoán hằng-vận-tốc càng trôi, nhưng KF báo
            # covariance quá tự tin -> phồng theo số frame chưa quan sát để (a) nới
            # cổng Mahalanobis cho phục hồi, (b) cho correlation appearance ghi đè
            # motion cũ. Track vừa update (dt=0) giữ nguyên.
            dt = max(self.frame_id - t.frame_id - 1, 0)
            if dt > 0:
                covs[i] = covs[i] * float((1 + dt) ** 2)

        # 2) correlation estimate cho track có template
        if self._dense_hat is not None:
            tmpls, idx = [], []
            for i, t in enumerate(pool):
                if t.template is not None:
            ...
            if idx:
                ...
                tt = torch.from_numpy(np.stack(tmpls)).to(self._dense_hat.device).float()
                tt = torch.nn.functional.normalize(tt, dim=1)
                cmap, covmap, peak = uam.predict_centers_cov(
                    tt, self._dense_hat, tau=self.uam_tau)
                cmap = cmap.cpu().numpy()
                covmap = covmap.cpu().numpy()
                peak = peak.cpu().numpy()
                c_orig = am.map_to_orig(cmap, stride, rx, ry, pw, ph)
                ccov_orig = uam.cov_map_to_orig(covmap, stride, rx, ry)
                # 3) inverse-variance fuse Kalman ⊕ correlation; phồng covariance
                #    correlation theo 1/peak -> match yếu (peak thấp) bị tin ít hơn.
                for k, i in enumerate(idx):
                    q = max(float(peak[k]), 0.1)
                    cc = ccov_orig[k] / q
                    x, P = uam.inv_var_fuse(means[i], covs[i], c_orig[k], cc)
                    means[i] = x
                    covs[i] = P
                    pool[i].pred_xy = x
        return means, covs
    ...
    # ----- main step -----
    def update(self, dets_per_class):
        """Một bước tracking. (Đã bỏ h_orig/w_orig vì không dùng.)

        Args:
            dets_per_class : dict[cls_id] -> list[MCTrack]
        Returns:
            dict[cls_id] -> list[MCTrack] track active.
        """
        self.frame_id += 1
        if self.frame_id == 1:
            MCTrack.init_count(self.num_classes)
        ...
        # GMC một lần / frame (nếu bật)
        gmc_H = None
        if self.gmc is not None and self._curr_img is not None:
            try:
                gmc_result = self.gmc.apply(self._curr_img, None)
                gmc_H = gmc_result[0] if isinstance(gmc_result, tuple) else gmc_result
            except Exception:
        ...
        for cls_id in range(self.num_classes):
            ...
            # Lấy template MOTION cho detection (chỉ khi có dense map).
            if self.use_am and self._dense_raw is not None:
                self._sample_det_templates(cls_detects)

            # --- Bước 1: association ---
            pool = track_pool_dict[cls_id]
            emb_d = matching.embedding_distance(pool, cls_detects)   # cosine (sparse emb)
            iou_d = matching.iou_distance(pool, cls_detects)

            if (self.use_am and self._dense_hat is not None
                    and len(pool) > 0 and len(cls_detects) > 0):
                # UAM: Kalman ⊕ correlation -> Mahalanobis gate + cosine cost.
                means, covs = self._uam_predict(pool)
                det_xy = self._centers_orig(cls_detects)
                cost = uam.maha_gate_cost(
                    means, covs, det_xy, emb_d,
                    chi2=self.uam_chi2, cos_thresh=self.uam_cos,
                    iou_dists=iou_d, iou_gate=self.uam_iou_gate)
                thr = self.uam_cos
            else:
                # Fallback: không có dense map -> IoU + appearance (không tham số phụ).
                cost = matching.fuse_score_three(iou_d, emb_d, cls_detects)
                thr = 0.6
            matches, u_track, u_detection = matching.linear_assignment(cost, thresh=thr)
