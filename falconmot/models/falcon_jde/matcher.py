...
def wasserstein_cost_matrix(out_bbox: torch.Tensor, tgt_bbox: torch.Tensor, eps: float = 1e-7, constant: float = 0.1) -> torch.Tensor:
    ...
    tgt_bbox: [M, 4] (cx, cy, w, h)
    
    Trả về ma trận kích thước [N, M] với giá trị âm (để đồng bộ với cost_giou).
    """
    # Unbind và thêm chiều (unsqueeze) để tự động broadcast thành ma trận [N, M]
    b1_cx, b1_cy, b1_w, b1_h = out_bbox.unsqueeze(1).unbind(-1)  # shape: [N, 1]
    b2_cx, b2_cy, b2_w, b2_h = tgt_bbox.unsqueeze(0).unbind(-1)  # shape: [1, M]

    # Khoảng cách Wasserstein bậc 2
    center_dist = (b1_cx - b2_cx)**2 + (b1_cy - b2_cy)**2
    wh_dist = ((b1_w / 2) - (b2_w / 2))**2 + ((b1_h / 2) - (b2_h / 2))**2
    wasserstein_2 = center_dist + wh_dist
    
    # NWD Similarity (từ 0 đến 1, 1 là trùng khít)
    nwd_sim = torch.exp(-torch.sqrt(wasserstein_2 + eps) / constant)
    
    # Matcher của DETR ưu tiên "Cost" càng nhỏ (âm) càng tốt
    return -nwd_sim
# =================================================================================
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs: Dict[str, torch.Tensor], targets, return_topk=False, epoch=0):
        ...
        if self.change_matcher and epoch >= self.matcher_change_epoch:
            ...
            # -------------------------------------------------------------------------------------
            # THÊM MỚI: Trộn NWD Similarity vào iou_order matching (D-FINE phase 2)
            # -------------------------------------------------------------------------------------
            nwd_sim = -wasserstein_cost_matrix(out_bbox, tgt_bbox) # nwd_sim trả về giá trị dương [0, 1]
            mixed_iou = 0.5 * bbox_iou + 0.5 * nwd_sim
            
            # Final cost matrix (Thay bbox_iou bằng mixed_iou)
            C = (-1) * (class_score * torch.pow(mixed_iou, self.iou_order_alpha))
            ...
            # -------------------------------------------------------------------------------------
            # THÊM MỚI: Trộn NWD Cost vào giai đoạn ghép cặp tiêu chuẩn
            # -------------------------------------------------------------------------------------
            cost_nwd = wasserstein_cost_matrix(out_bbox, tgt_bbox) # Trả về số âm
            cost_iou_mixed = 0.5 * cost_giou + 0.5 * cost_nwd

            # Final cost matrix 3 * self.cost_bbox + 2 * self.cost_class + self.cost_giou
            # Dùng cost_iou_mixed thay cho cost_giou
            C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_iou_mixed
