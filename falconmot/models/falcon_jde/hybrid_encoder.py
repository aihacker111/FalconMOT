...
class RepNCSPELAN4(nn.Module):
    def __init__(self, c1, c2, c3, c4, n=3, bias=False, act="silu", csp_type='csp2'):
        ...
        CSPLayer_block = CSPLayer2 if csp_type == 'csp2' else CSPLayer
        self.cv2 = nn.Sequential(CSPLayer_block(c3//2, c4, n, 1, bias=bias, act=act, bottletype=VGGBlock), ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act))
        self.cv3 = nn.Sequential(CSPLayer_block(c4, c4, n, 1, bias=bias, act=act, bottletype=VGGBlock), ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act))
...
# ─── ĐOẠN MÃ CẢI TIẾN: HIGH-RES LEARNED UPSAMPLE & CONTENT-AWARE GATE ───

class LightUpsample(nn.Module):
    """Bilinear x2 upsample (0 param, ~0 FLOP) - đúng kiểu RT-DETR/DEIM.
    Thay cho PixelShuffle học được (conv 192->768 3x3) vốn chiếm ~2.66M param."""
...
class PerPixelGatedFusion(nn.Module):
    """Cổng trộn nhẹ per-pixel (content-aware): chỉ 1 conv 1x1 -> 1 kênh gate.
    Bỏ proj 3x3 thừa (fpn_blocks/pan_blocks RepNCSPELAN ngay sau đã fuse bằng conv)
    và bỏ gate 3x3 nặng. Giảm ~2.66M param + nhiều FLOP, vẫn giữ gating theo nội dung."""
...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
                 in_channels=[256, 256, 512, 1024], # Hỗ trợ linh hoạt cả 3 hoặc 4 mức tùy biến (S4)
        ...
        # Channel projection đưa mọi scale đầu vào về hidden_dim
        ...
        if version == 'dfine':
            ...
            Fuse_Block = CSPLayer(in_channels=c1, out_channels=c2, num_blocks=num_blocks, act=act, expansion=expansion, bottletype=VGGBlock)
        ...
        # Tích hợp khối học nâng độ phân giải và cổng lọc đồng bộ nội dung
        self.upsamplers = nn.ModuleList([
            LightUpsample(scale_factor=2) for _ in range(len(in_channels) - 1)
        ])
        self.gated_fusions_td = nn.ModuleList([
            PerPixelGatedFusion(hidden_dim) for _ in range(len(in_channels) - 1)
        ])
        self.gated_fusions_bu = nn.ModuleList([
            PerPixelGatedFusion(hidden_dim) for _ in range(len(in_channels) - 1)
        ])
    ...
    def forward(self, feats):
        ...
        # Chạy khối Transformer cho các lớp chỉ định (ví dụ: S32)
        ...
        # ─── 1. TOP-DOWN FPN LOOP (Có tích hợp PixelShuffle + Gated Fusion) ───
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            # Bilinear upsample (nhẹ) để khôi phục chiều không gian
            upsample_feat = self.upsamplers[len(self.in_channels) - 1 - idx](feat_heigh)
            
            # Sử dụng Content-Aware Gated Fusion thay vì phép cộng ma trận vô hướng
            fused_gated = self.gated_fusions_td[len(self.in_channels) - 1 - idx](upsample_feat, feat_low)
            
            if self.fuse_op == 'sum':
                fused_feat = fused_gated
            else:
                fused_feat = torch.concat([fused_gated, feat_low], dim=1)
                
            inner_out = self.fpn_blocks[len(self.in_channels) - 1 - idx](fused_feat)
            inner_outs.insert(0, inner_out)

        # ─── 2. BOTTOM-UP PAN LOOP (Có bổ trợ Gated Fusion) ───
        outs = [inner_outs[0]]
        for idx in range(len(self.in_channels) - 1):
            ...
            # Gated Fusion cho pha Bottom-Up nhằm lọc bớt noise tầng nông
            fused_gated = self.gated_fusions_bu[idx](downsample_feat, feat_height)
            
            if self.fuse_op == 'sum':
                fused_feat = fused_gated
            else:
                fused_feat = torch.concat([fused_gated, feat_height], dim=1)
