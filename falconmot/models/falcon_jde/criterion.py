...
class FalconJDECriterion(nn.Module):
    ...
    def loss_labels_focal(self, outputs, targets, indices, num_boxes):
        """Classification loss dispatcher.

        self.cls_loss_type:
          'focal'  — vanilla sigmoid focal (one-hot hard target).
          'ia_bce' — IoU-aware BCE (RF-DETR / VFL style): positive target is
                     modulated by the predicted-vs-GT IoU, so high-IoU boxes are
                     pushed to high confidence. Improves AP at high IoU and
                     calibration. This is the default.
        """
        if self.cls_loss_type == 'focal':
            return self._loss_cls_vanilla_focal(outputs, targets, indices, num_boxes)
        return self._loss_cls_ia_bce(outputs, targets, indices, num_boxes)

    def _loss_cls_vanilla_focal(self, outputs, targets, indices, num_boxes):
        src_logits = outputs['pred_logits']
        idx = self._get_src_permutation_idx(indices)
        target_classes_o = torch.cat([t['labels'][J] for t, (_, J) in zip(targets, indices)])
        target_classes = torch.full(src_logits.shape[:2], self.num_classes,
    ...
    def _loss_cls_ia_bce(self, outputs, targets, indices, num_boxes):
        """IoU-aware BCE classification loss (RF-DETR ia_bce / VFL family).

        Positive-location target is t = prob^alpha * iou^(1-alpha) (clamped,
        detached); negatives keep a focal-style prob^gamma weight. The BCE is
        written via logsigmoid for numerical stability and normalised by
        num_boxes — matching the vanilla-focal normalisation scale.
        """
        ...
        if target_classes_o.numel() > 0:
            ...
            pos_ious = torch.diag(box_iou(
                box_cxcywh_to_xyxy(src_boxes.detach()),
                box_cxcywh_to_xyxy(target_boxes))[0]).detach()

            pos_ind = list(idx) + [target_classes_o]               # (b, q, c) index
            t = prob[tuple(pos_ind)].pow(alpha) * pos_ious.pow(1 - alpha)
            t = torch.clamp(t, 0.01).detach()
            pos_weights[tuple(pos_ind)] = t.to(pos_weights.dtype)
            neg_weights[tuple(pos_ind)] = (1 - t).to(neg_weights.dtype)

        # loss = -pos·log(p) - neg·log(1-p), rewritten via logsigmoid:
        loss = neg_weights * src_logits \
