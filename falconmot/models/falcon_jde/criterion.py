...
# Detection (khớp đúng config `deimv2_dinov3_s`):
...
# ReID (phần mở rộng MOT của riêng dự án, giữ nguyên):
#     loss_reid            — per-class CE/ArcFace (+ Triplet, + dense alignment tuỳ chọn)
#     loss_s4_aux          — heatmap phụ stride-4 (tuỳ chọn)

# Tổng loss cân bằng det/ReID bằng homoscedastic-uncertainty (Kendall et al.).
# """
...
#     """ArcFace margin loss, hạ s/m cho vật thể nhỏ & nhoè (drone)."""
...
#         mal_alpha:           float = None,           # None = theo DEIM gốc (deimv2_s)
#         reg_max:             int   = 32,
...
#         # Cache cho FGL/DDF, reset mỗi forward.
...
#             # FairMOT/AMOT embedding scale theo số ID mỗi lớp.
#             self.emb_scale_dict = {
...
#         """Matchability-Aware Loss (DEIM): target mềm = IoU^gamma tại lớp GT."""
...
#         """CE trên 2 bin lân cận (trái/phải) với trọng số nội suy — nhân FGL của D-FINE."""
#         dis_left = label.long()
...
#         Giám sát TRỰC TIẾP phân phối rời rạc `pred_corners` (4*(reg_max+1)).
#         Bỏ qua nhánh không có corners (enc/pre)."""
#         losses = {}
...
#         # Target phân phối: tính 1 lần/forward (tách riêng nhánh denoising).
#         with torch.no_grad():
...
#         # DDF: distill phân phối tầng cuối (teacher) -> các tầng trước.
...
#                 # num_pos/num_neg tính 1 lần ở nhánh non-dn, tái dùng cho dn.
...
#         """Bilinear-sample emb_map [D,H,W] tại tâm GT (cx,cy ∈ [0,1]) -> [n, D]."""
#         D = emb_map_b.shape[0]
...
#         """ReID per-class: sparse CE(+Triplet) + dense alignment (nếu có pred_reid_map)."""
...
#             # dense CE + consistency về sparse
...
#         """Union set (Dense O2O) qua tất cả các tầng decoder."""
...
#         """boxes_weight (IoU) cho loss_boxes khi bật boxes_weight_format."""
...
#                 # s4_aux chỉ tính 1 lần ở nhánh chính.
#                 if loss == 's4_aux' and suffix != '':
#                     continue
#                 # GO/union set áp cho cả boxes và local (theo DEIM).
...
#         # Nhánh chính (last layer) — set self.fgl_targets cho FGL.
...
#         # Pre head (D-FINE), không có corners -> chỉ mal/boxes.
#         if pre_list:
#             _apply(pre_list[0], targets, cached_indices[len(aux_list)], indices_go,
#                    num_boxes, num_boxes_go, '_pre')

#         # Encoder aux (class-agnostic nếu cần), không có corners.
...
#         # Denoising (CDN) — đánh dấu is_dn để FGL dùng cache riêng.
...
#         # ------------------------------------------------------------------
#         # Tổng loss — uncertainty weighting (Kendall et al.).
#         # det_loss gộp mọi term detection (main + aux + dn + enc + s4);
#         # chỉ ReID được cân bằng riêng. (s_det + s_id) là regulariser.
#         # ------------------------------------------------------------------
...
class FalconJDECriterion(nn.Module):
    ...
    def forward(self, outputs, targets, epoch: int = 0):
        ...
        # ------------------------------------------------------------------
        # Logging: hiển thị TỪNG thành phần riêng để dễ theo dõi.
        #   loss_mal, loss_mal_aux_0, loss_mal_aux_1, ...
        #   loss_fgl, loss_fgl_aux_0, ...   (KHÔNG gộp tổng)
        # Các key này đã được _apply() tạo sẵn theo suffix; ở đây chỉ detach
        # để in ra (không phá graph — 'loss' tổng vẫn giữ nguyên cho backward).
        # ------------------------------------------------------------------
        for k in list(losses.keys()):
            if k != 'loss' and isinstance(losses[k], torch.Tensor):
                losses[k] = losses[k].detach()

        # Đảm bảo các key chính luôn tồn tại để Trainer không KeyError.
        # (loss_ddf ở nhánh chính không có vì tầng cuối đóng vai teacher → mặc định 0)
