...
#     """Overwrite the region [y0:y0+eh, x0:x0+ew] of a BGR uint8 image according to `mode`.

#     mode:
#       'random' : uniform noise 0..255 (standard Random Erasing).
#       'mean'   : gray 114 (the letterbox value used in this repo).
#       'patch'  : copy another random region from the same image -> an occluder that
#                  matches real scene content (more natural than noise, ideal for drone views).
#     """
#     h, w = img.shape[:2]
...
#                            min_obj_px=12):
#     """Partially cover (an edge strip of) some objects to simulate occlusion for ReID/MOT.

#     Uses an edge strip (top/bottom/left/right) with frac < 0.5, so the box center is never
#     covered -> safe for dense ReID (sampling at the GT center still hits a visible part).

#     img    : BGR uint8 (H, W, 3) at network resolution.
#     labels : [N,6] = [cls, tid, cx, cy, w, h] normalized to [0,1]. Not modified.
#     p              : probability of applying this to the whole image.
#     occ_obj_frac   : fraction of objects occluded per image.
#     frac_lo/hi     : fraction of the box size covered along one axis (forced < 0.5).
#     mode           : 'patch' | 'random' | 'mean' (xem _fill_region).
#     min_obj_px     : skip objects with a side smaller than this (occluding would erase them).
#     """
...
#     frac_hi = min(frac_hi, 0.49)   # ensure the box center is always visible
...
#     """Image-level Random Erasing (Zhong et al. 2020). Labels are unchanged.

#     sl,sh : range of the erased-area / image-area ratio.
...
#                            scale_min=0.3, scale_max=0.6, fill_value=114): # Mặc định nên để xám 114
#     # SỬA LỖI 1: Tạo canvas đúng fill_value
...
#         # Tính tỷ lệ thu nhỏ để ảnh nhét vừa vào tile mà KHÔNG BỊ MÉO
#         r = min(tile_w / orig_w, tile_h / orig_h)
        
#         if random.random() < scale_bias_prob:
#             # Nhỏ hơn nữa để tạo khoảng trống (bias)
#             scale = random.uniform(scale_min, scale_max) * r
#         else:
#             # Vừa khít nhất có thể (giữ đúng aspect ratio)
#             scale = r 

#         small_w = max(4, int(orig_w * scale))
#         small_h = max(4, int(orig_h * scale))
#         small = cv2.resize(img, (small_w, small_h), interpolation=cv2.INTER_AREA)

#         # Đặt vị trí ngẫu nhiên bên trong tile
#         ox = random.randint(0, tile_w - small_w)
#         oy = random.randint(0, tile_h - small_h)

#         # SỬA LỖI 1: Tile cũng phải pad màu fill_value
#         tile = np.full((tile_h, tile_w, 3), fill_value, dtype=np.uint8)
#         tile[oy:oy + small_h, ox:ox + small_w] = small

#         # Tính lại nhãn chuẩn cho tile
#         if len(labels) > 0:
#             lbs = labels.copy()
...
#         # Dán tile vào mosaic lớn
#         mosaic[ty1:ty2, tx1:tx2] = tile

#         # Chuyển nhãn lên hệ quy chiếu toàn cục (Global)
#         if len(lbs) > 0:
#             ml = lbs.copy()
...
# [FIXED]: fill_value=114
def random_zoom_out(img, labels, max_scale=2.0, fill_value=114, p=0.5):
    """Place the image on a larger canvas (zoom out), simulating altitude gain."""
    if random.random() > p:
        return img, labels
...
def copy_paste_small_objects(img, labels, max_area=0.01, max_paste=5, p=0.5):
    ...
    for idx in chosen:
        ...
        # [FIXED]: Clamp safe boundary
        new_lb[2] = max(0.0, min((dx + pw / 2.0) / w, 1.0))
        new_lb[3] = max(0.0, min((dy + ph / 2.0) / h, 1.0))
...
# [FIXED]: Pad màu xám (114) và GIỮ NGUYÊN TỶ LỆ KHUNG HÌNH để bảo vệ ReID Feature
def mosaic_with_scale_bias(imgs_labels, output_w, output_h, scale_bias_prob=0.5,
                           scale_min=0.3, scale_max=0.6, fill_value=114): 
    mosaic = np.full((output_h, output_w, 3), fill_value, dtype=np.uint8)
    mid_x  = output_w // 2
    mid_y  = output_h // 2
    ...
    for (img, labels), (tx1, ty1, tx2, ty2) in zip(imgs_labels, placements):
        ...
        # Tile cũng phải pad màu fill_value
        tile = np.full((tile_h, tile_w, 3), fill_value, dtype=np.uint8)
        tile[oy:oy + small_h, ox:ox + small_w] = small
...
# [FIXED]: borderValue=(114, 114, 114)
def random_affine(img, targets=None, degrees=(-5, 5), translate=(0.10, 0.10),
                  scale=(0.50, 1.20), shear=(-2, 2), borderValue=(114, 114, 114)):
    border = 0
    height = img.shape[0]
    width  = img.shape[1]
...
# [FIXED]: borderValue=(114, 114, 114)
def random_homography_warp(img, targets=None, strength=0.12, borderValue=(114, 114, 114), min_box=4):
    h, w = img.shape[:2]
    ang = strength * 110.0
    pitch = np.random.uniform(-ang, ang)
    yaw   = np.random.uniform(-ang, ang)
