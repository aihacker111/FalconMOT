...
Augmentation pipeline (NO LETTERBOX):
  ...
  2. random_zoom_out : expand the canvas (simulates a high-flying drone, black border)
  3. random_crop     : randomly crop a region of the image
  4. resize          : plain resize to 960x544
  5. random_affine / homography : geometric transforms (black border)
...
from falconmot.data.augment import (
...
class VisDroneCocoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, opt, img_root: str = None, ann_file: str = None,
        ...
        # Read size from opt, falling back to 960x544
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if not mosaic_used:
            ...
            # 2. Random zoom-out (canvas expand - black fill, fill_value=0)
            if do_aug and self.use_zoom_out:
                img, labels = random_zoom_out(img, labels, max_scale=1.5, fill_value=0, p=self.zoom_out_prob)

            # 4. Plain resize to network size (960, 544)
            img = cv2.resize(img, (self.width, self.height), interpolation=cv2.INTER_AREA)

            # 5. Convert labels to pixels in the new canvas coordinates (960, 544)
            if len(labels) > 0:
                lbs = labels.copy()
                lbs[:, 2] = (labels[:, 2] - labels[:, 4] * 0.5) * self.width
                lbs[:, 3] = (labels[:, 3] - labels[:, 5] * 0.5) * self.height
        ...
        # -- Geometric transforms (automatic black border) --
        if do_aug:
        ...
        return {
            ...
            'lb_pad':         np.array([0, 0], dtype=np.int32),  # no padding
        }

# ---------------------------------------------------------------------------
# The tracking loaders also set the default size to (960, 544)
# ---------------------------------------------------------------------------
