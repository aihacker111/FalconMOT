...
from .falcon_jde.model import build_falcon_jde, FalconJDEModel


def create_model(arch: str, opt) -> FalconJDEModel:
    """Create detection model.

    Supported arch values:
        'falcon_jde'  — DINOv3STAs + HybridEncoder + DEIMTransformer + ReID head
    """
    if arch.startswith('falcon_jde'):
        return build_falcon_jde(opt)
    raise ValueError(f"Unknown arch '{arch}'. Supported: 'falcon_jde'")
        ...
        if 'optimizer' in checkpoint:
            try:
                ...
                print(f'[Warning] Cannot load optimizer state ({e}). '
                      f'Starting with fresh optimizer but restoring epoch.')
