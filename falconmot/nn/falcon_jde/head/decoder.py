...
from falconmot.nn.falcon_jde.head.denoising import get_contrastive_denoising_training_group
from ..ops.utils import deformable_attention_core_func_v2, get_activation, inverse_sigmoid, bias_init_with_prob

from .dfine_decoder import MSDeformableAttention, LQE, Integral
from ..ops.utils import weighting_function, distance2bbox
from ..ops.utils import RMSNorm, SwiGLUFFN, Gate, MLP
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        # eval_hs: hidden states at the eval layer (last appended output)
        eval_hs = output  # (B, N_total, hidden_dim)
        return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), \
               torch.stack(dec_out_pred_corners), torch.stack(dec_out_refs), pre_bboxes, pre_scores, eval_hs
...
class DEIMTransformer(nn.Module):
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        out_bboxes, out_logits, out_corners, out_refs, pre_bboxes, pre_logits, eval_hs = self.decoder(
            init_ref_contents,
        ...
        # Slice detection queries only (strip DN queries which are prepended)
        det_hs = eval_hs[:, -self.num_queries:, :]   # (B, N_det, hidden_dim)
        ...
        if self.training:
            ...
            out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1], 'eval_hs': det_hs}
