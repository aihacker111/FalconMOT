...
class DenseReIDHead(nn.Module):
    """TOD-Head — Task-Orthogonal Deformable ReID head.

    Backward-compatible with the previous DenseReIDHead (all original sub-module
    names are preserved, so old checkpoints still load).  When ``use_tod=True``
    it additionally realises the three pillars of the Task-Orthogonal Deformable
    Head:

      1. Siamese Task Queries — the SHARED object query is projected by two
         independent linears into a detection sub-space (``q_det``) and a ReID
         sub-space (``q_proj`` == Q_reid).  ``q_proj`` steers the ReID
         deformable sampling; ``q_det`` produces the detection-side feature used
         by the orthogonal constraint.

      2. Task-Specific Deformable Sampling with Guided Anchors — the ReID branch
         runs its own ``MSDeformableAttention``.  Its reference points are the
         DETACHED detection boxes whose (w, h) are shrunk by ``box_shrink`` so
         the sampling points stay inside the object interior (face / torso /
         logo) instead of leaking onto background or a neighbouring instance in
         crowded / occluded scenes (Asymmetric Gradient Isolation: geometry is
         transferred one-way through ``.detach()`` upstream).

      3. Orthogonal Feature Loss support — the head exposes ``emb_ortho_det``
         (F_det) and ``emb_ortho_reid`` (F_reid).  The criterion drives their
         normalised cross-correlation to zero, forcing the shared trunk to learn
         two disjoint filter clusters (shape/edges vs. texture/colour).

    Also exposes ``emb_app`` so the dense ``cons`` loss does not conflict with
    ``dense_ce``.
    """
    def __init__(self, hidden_dim, reid_dim, num_heads=8, num_points=8,
                 use_s4_dense=False, s4_in_ch=None,
                 tower_depth=2, query_gate_init=0.1, detach_input=True,
                 # ----- TOD-Head (Task-Orthogonal Deformable) -----
                 use_tod=False, box_shrink=0.5, ortho_dim=None,
                 **kwargs):
        super().__init__()
        nh = num_heads if reid_dim % num_heads == 0 else _largest_divisor(reid_dim)
        self.hidden_dim    = hidden_dim
        ...
        self.box_shrink    = float(box_shrink)
        self.ortho_dim     = int(ortho_dim) if ortho_dim else reid_dim
        ...
        # SPARSE PATH — the query only locates the sampling points.
        # NOTE: `q_proj` is the Siamese ReID task query (Q_reid). The name is kept
        ...
        # ----- Pillar 1 + 3: Siamese detection query + orthogonal projections -----
        # These are only instantiated under TOD so a non-TOD checkpoint stays clean.
        if self.use_tod:
            # Q_det — detection-task projection of the SHARED object query.
            self.q_det = nn.Linear(hidden_dim, self.ortho_dim)
            # Project the two task streams into a common comparison sub-space.
            self.ortho_proj_det  = nn.Linear(self.ortho_dim, self.ortho_dim, bias=False)
            self.ortho_proj_reid = nn.Linear(reid_dim,       self.ortho_dim, bias=False)

    @staticmethod
    def _shrink_boxes(boxes, gamma):
        """Shrink box (w, h) toward the centre by `gamma` (cx, cy unchanged).

        boxes: (..., 4) in normalised cxcywh. Keeps ReID deformable sampling
        inside the object interior — the guided-anchor mechanism.
        """
        if gamma == 1.0:
            return boxes
        cxcy, wh = boxes[..., :2], boxes[..., 2:]
        return torch.cat([cxcy, wh * gamma], dim=-1)
    ...
    def forward(self, query, boxes, reid_feat, c1=None, return_dense=False,
        ...
        """
        Args:
            query     : detached object query (POINTER only).
            boxes     : detached detection boxes (cxcywh, normalised) — guided anchors.
            reid_feat : shared trunk feature (gradient coupling controlled upstream).
            query_raw : NON-detached object query, used ONLY by the orthogonal
                        constraint's detection projection (q_det). Falls back to
                        `query` when None (e.g. inference).
        """
        if self.detach_input:
            reid_feat = reid_feat.detach()
            if c1 is not None:
        ...
        # Pillar 2: guided anchors — shrink the (detached) box so ReID samples
        # the interior, not the edges/background.
        ref_boxes = boxes
        if self.use_tod and self.box_shrink != 1.0:
            ref_boxes = self._shrink_boxes(boxes, self.box_shrink)

        q_in = self.norm_q(self.q_proj(query))                 # Q_reid steers sampling
        app_raw = self.deform_attn(q_in, ref_boxes.unsqueeze(2), value_list, spatial_shapes)
        app = self.norm_attn(app_raw)
        ...
        # Pillar 1 + 3: expose the two task features for the orthogonal loss.
        if self.use_tod:
            q_for_det = query_raw if query_raw is not None else query
            f_det  = self.ortho_proj_det(self.q_det(q_for_det))   # (B,N,ortho_dim)
            f_reid = self.ortho_proj_reid(app_raw)                # (B,N,ortho_dim)
            out['emb_ortho_det']  = f_det
            out['emb_ortho_reid'] = f_reid
...
# Backward/forward-friendly alias — the academic name of the head.
TODReIDHead = DenseReIDHead
...
class FalconJDEModel(nn.Module):
    def __init__(
        ...
        if use_reid:
            # Under TOD we want a controlled gradient path from the ReID stream
            # into the shared trunk (so the orthogonal loss can actually
            # disentangle the backbone). We therefore couple the value path
            # (detach_input=False) and damp it with `reid_grad_scale`; the
            # geometry stays one-way detached upstream. Without TOD we keep the
            # previous fully read-only behaviour (detach_input=True).
            self.reid_head = DenseReIDHead(
                decoder.hidden_dim, reid_dim,
                num_heads=8, num_points=reid_num_points,
                use_s4_dense=reid_use_s4_dense,
                s4_in_ch=reid_s4_in_ch,
                detach_input=(not self.use_tod),
                use_tod=self.use_tod,
                box_shrink=reid_box_shrink,
                ortho_dim=ortho_dim,
            )
    ...
    def forward(self, x: torch.Tensor, targets=None):
        ...
        if 'eval_hs' in out and self.use_reid:
            ...
            # Geometry transfer is ALWAYS one-way (detach): the ReID gradient must
            # never corrupt the decoder's localisation. Under TOD we additionally
            # feed the NON-detached query (`query_raw`) used solely by the
            # orthogonal constraint's detection projection, and we couple the
            # trunk feature with a damped gradient so the orthogonal loss can
            # disentangle the backbone.
            if self.use_tod:
                reid_feat_in = grad_scale(reid_feat, self.reid_grad_scale)
                c1_in = grad_scale(c1, self.reid_grad_scale) if c1 is not None else None
                query_raw = hs
            else:
                reid_feat_in = reid_feat
                c1_in = c1
                query_raw = None

            reid_out = self.reid_head(
                hs.detach(), pred_boxes.detach(), reid_feat_in,
                c1=c1_in, return_dense=want_dense, query_raw=query_raw,
            )
            out['pred_reid']     = reid_out['emb']
            out['pred_reid_raw'] = reid_out['emb_raw']
            if self.training:
                out['pred_reid_app'] = reid_out['emb_app']
                if 'emb_ortho_det' in reid_out:
                    out['pred_reid_ortho_det']  = reid_out['emb_ortho_det']
                    out['pred_reid_ortho_reid'] = reid_out['emb_ortho_reid']
            if 'emb_map' in reid_out:
                if self.training:
                    out['pred_reid_map'] = reid_out['emb_map']
                if getattr(self, 'return_reid_dense', False) and not self.training:
...
def build_falcon_jde(opt) -> FalconJDEModel:
    ...
    model = FalconJDEModel(
        ...
        reid_box_shrink=getattr(opt, 'reid_box_shrink', 0.5),
        ortho_dim=getattr(opt, 'ortho_dim', None),
    )
