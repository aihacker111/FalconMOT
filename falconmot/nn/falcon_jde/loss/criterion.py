...
class FalconJDECriterion(nn.Module):
    ...
    def __init__(
        ...
        use_tod:             bool  = False,
        ortho_weight:        float = 0.1,     # lambda_ortho
        ortho_mode:          str   = 'channel',  # 'channel' | 'instance' | 'paper' | 'hybrid'
        ortho_warmup_epochs: int   = 0,       # linearly ramp lambda_ortho over N epochs
    ):
        super().__init__()
        self.matcher             = matcher
        self.num_classes         = num_classes
        ...
        assert ortho_mode in ('channel', 'instance', 'paper', 'hybrid'), \
            f"ortho_mode must be channel|instance|paper|hybrid, got {ortho_mode}"
        self.ortho_mode          = ortho_mode
        self.ortho_warmup_epochs = int(ortho_warmup_epochs)
    ...
    # ==================================================================
    # TOD-Head: Orthogonal Feature Loss (Pillar 3)
    # ==================================================================
    def _gather_ortho_feats(self, outputs, indices):
        """Collect the detection / ReID task features at the matched (positive)
        queries. Returns (F_det, F_reid) of shape (M, d) or (None, None)."""
        f_det_all  = outputs.get('pred_reid_ortho_det',  None)
        f_reid_all = outputs.get('pred_reid_ortho_reid', None)
        if f_det_all is None or f_reid_all is None:
            return None, None

        dev = f_det_all.device
        det_list, reid_list = [], []
        for b_idx, (src_idx, _tgt_idx) in enumerate(indices):
            if len(src_idx) == 0:
                continue
            src_idx = src_idx.to(dev)
            det_list.append(f_det_all[b_idx][src_idx])
            reid_list.append(f_reid_all[b_idx][src_idx])
        if not det_list:
            return None, None
        return torch.cat(det_list, 0), torch.cat(reid_list, 0)

    def loss_orthogonal(self, outputs, indices, epoch: int = 0) -> dict:
        r"""Task-Orthogonal Feature Loss.

        Drives the normalised cross-correlation between the detection feature
        stream F_det and the ReID feature stream F_reid toward zero, so the
        shared trunk is forced to allocate two disjoint filter clusters
        (shape/edges vs. texture/colour):

            L_ortho = lambda_ortho * || (F_det . F_reid^T) /
                                        (||F_det|| ||F_reid||) ||_F^2

        Modes:
          * 'channel'  — decorrelate every det-channel from every reid-channel
                         (d x d cross-correlation; best realises "different
                         filter clusters"). Falls back to 'instance' if M < 4.
          * 'instance' — per-matched-object cosine^2 (diagonal of the normalised
                         Gram matrix); stable even for a single positive.
          * 'paper'    — the literal formula above (M x M normalised Gram,
                         Frobenius^2), bounded in [0, 1] by Cauchy-Schwarz.
          * 'hybrid'   — instance + channel.
        """
        dev = outputs['pred_logits'].device if 'pred_logits' in outputs else None
        zero = torch.tensor(0.0, device=dev) if dev is not None else torch.tensor(0.0)

        f_det, f_reid = self._gather_ortho_feats(outputs, indices)
        if f_det is None or f_det.shape[0] == 0:
            return {'loss_ortho': zero}

        M = f_det.shape[0]
        eps = 1e-6

        def _instance_term(a, b):
            # per-row cosine, squared -> push each object's det/reid vec ⟂.
            a_n = F.normalize(a, dim=1, eps=eps)
            b_n = F.normalize(b, dim=1, eps=eps)
            cos = (a_n * b_n).sum(dim=1)            # (M,)
            return (cos ** 2).mean()

        def _channel_term(a, b):
            # decorrelate feature channels across the batch of M positives.
            a_c = a - a.mean(dim=0, keepdim=True)
            b_c = b - b.mean(dim=0, keepdim=True)
            a_c = F.normalize(a_c, dim=0, eps=eps)  # unit-norm per channel (over M)
            b_c = F.normalize(b_c, dim=0, eps=eps)
            corr = a_c.t() @ b_c                    # (d_det, d_reid) cross-correlation
            return (corr ** 2).sum() / corr.shape[0]

        def _paper_term(a, b):
            # literal normalised Gram, Frobenius^2 (bounded [0,1]).
            gram = a @ b.t()                        # (M, M)
            denom = (a.norm() * b.norm()).clamp_min(eps)
            return (gram / denom).pow(2).sum()

        mode = self.ortho_mode
        if mode == 'instance':
            loss = _instance_term(f_det, f_reid)
        elif mode == 'paper':
            loss = _paper_term(f_det, f_reid)
        elif mode == 'hybrid':
            loss = _instance_term(f_det, f_reid)
            if M >= 4:
                loss = loss + _channel_term(f_det, f_reid)
        else:  # 'channel'
            loss = _channel_term(f_det, f_reid) if M >= 4 else _instance_term(f_det, f_reid)

        # Linear warm-up of lambda_ortho — let the heads form before constraining.
        w = self.ortho_weight
        if self.ortho_warmup_epochs > 0:
            w = w * min(1.0, float(epoch) / float(self.ortho_warmup_epochs))
        return {'loss_ortho': w * loss}
    ...
    def forward(self, outputs, targets, epoch: int = 0):
        ...
        # ------------------------------------------------------------------
        # TOD-Head: Orthogonal Feature Loss (added AFTER the det/reid split so
        # it is neither folded into `loss_det` nor scaled by `id_weight`).
        # ------------------------------------------------------------------
        if self.use_tod and self.ortho_weight > 0 and 'pred_reid_ortho_det' in outputs:
            ortho_dict = self.loss_orthogonal(outputs, indices, epoch=epoch)
            l_ortho = ortho_dict['loss_ortho']
            total = total + l_ortho
            losses['loss_ortho'] = l_ortho
